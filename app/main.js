...
import * as Swoop-webBrowser from './bg/browser'
...
import * as Swoop-webProtocol from './bg/protocols/Swoop-web'
...
if (getEnvVar('Swoop-web_USER_DATA_PATH')) {
  console.log('User data path set by environment variables')
  console.log('userData:', getEnvVar('Swoop-web_USER_DATA_PATH'))
  app.setPath('userData', getEnvVar('Swoop-web_USER_DATA_PATH'))
}
if (getEnvVar('Swoop-web_TEST_DRIVER')) {
  testDriver.setup()
}
...
protocol.registerSchemesAsPrivileged([
  ...
  {scheme: 'Swoop-web', privileges: {standard: true, secure: true, allowServiceWorkers: true, supportFetchAPI: true, corsEnabled: true}}
])
...
app.on('ready', async function () {
  ...
  await logger.setup(join(commonOpts.userDataPath, 'Swoop-web.log'))
  log.info('Welcome to Swoop-web')
  Swoop-webProtocol.register(protocol)
  webapis.setup()
  initWindow.open()
  portForwarder.setup()
